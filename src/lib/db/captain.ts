...
export function getConversation(id: string, orgId?: string): CaptainConversation | undefined {
  const db = getDb();
  ...
  return db.prepare(`SELECT * FROM captain_conversations WHERE id = ?`).get(id) as
    | CaptainConversation
    | undefined;
}
...
export function updateConversation(
  ...
  db.prepare(`UPDATE captain_conversations SET ${sets.join(", ")} WHERE id = ?`).run(...vals);
}
...
export function listMessages(conversationId: string): CaptainMessage[] {
  const db = getDb();
  return db
    .prepare(`SELECT * FROM captain_messages WHERE conversation_id = ? ORDER BY created_at ASC`)
...
export function createMessage(
  ...
  db.prepare(`UPDATE captain_conversations SET updated_at = unixepoch() WHERE id = ?`).run(
    conversationId,
  );
  return db.prepare(`SELECT * FROM captain_messages WHERE id = ?`).get(id) as CaptainMessage;
}

export function updateMessageContent(id: string, content: string) {
  const db = getDb();
  db.prepare(`UPDATE captain_messages SET content = ? WHERE id = ?`).run(content, id);
}
...
export function listCaptainOutput(
  ...
  return db
    .prepare(`SELECT * FROM captain_output WHERE conversation_id = ? AND id > ? ORDER BY id ASC`)
...
export function deleteCaptainOutput(conversationId: string) {
  const db = getDb();
  db.prepare(`DELETE FROM captain_output WHERE conversation_id = ?`).run(conversationId);
}
