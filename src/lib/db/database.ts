...
export function createDatabase(
  ...
  name: string,
  columns: ColumnDef[],
): DatabaseMeta & { columns: ColumnInfo[] } {
  const db = getDb();
  const id = uuid();
...
export function getDatabaseById(id: string): (DatabaseMeta & { columns: ColumnInfo[] }) | null {
  const db = getDb();
  const meta = db.prepare(`SELECT * FROM databases WHERE id = ?`).get(id) as
    | DatabaseMeta
    | undefined;
  if (!meta) return null;
 ...
 */
export function getDatabaseByName(
  orgId: string,
  projectId: string | null,
  name: string,
): (DatabaseMeta & { columns: ColumnInfo[] }) | null {
  const db = getDb();
  const safeName = sanitizeName(name);
  const meta = db
    .prepare(`
...
export function deleteDatabase(id: string) {
  const db = getDb();
  const meta = db.prepare(`SELECT table_name FROM databases WHERE id = ?`).get(id) as
    | { table_name: string }
    | undefined;
  if (!meta) return;
...
export function addColumn(databaseId: string, column: ColumnDef) {
  const db = getDb();
  const meta = db.prepare(`SELECT * FROM databases WHERE id = ?`).get(databaseId) as
    | DatabaseMeta
    | undefined;
  ...
  const version =
    (
      db
        .prepare(`SELECT MAX(version) as v FROM database_migrations WHERE database_id = ?`)
        .get(databaseId) as { v: number }
    )?.v || 0;
...
export function getDatabaseMigrations(databaseId: string) {
  const db = getDb();
  return db
    .prepare(`SELECT * FROM database_migrations WHERE database_id = ? ORDER BY version ASC`)
    .all(databaseId);
}
...
export function getRows(
  ...
  const db = getDb();
  const meta = db.prepare(`SELECT table_name FROM databases WHERE id = ?`).get(databaseId) as
    | { table_name: string }
    | undefined;
  if (!meta) return null;
...
export function unlinkDatabaseFromJob(jobId: string, databaseId: string) {
  const db = getDb();
  db.prepare(`DELETE FROM job_databases WHERE job_id = ? AND database_id = ?`).run(
    jobId,
    databaseId,
  );
}
