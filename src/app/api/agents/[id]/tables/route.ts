...
export const POST = withAgentOrUser(async (req, auth, { params }) => {
  ...
  // own work). Users may act on any agent.
  ...
  try {
    // Get or create the table (scoped to the agent's project).
    let table = getTableByName(agent.project_id, name);
    const created = !table;
    if (!table) {
      if (!columns?.length) {
      ...
      table = createTable(agent.project_id, name, columns);
    }
