...
// The local runner is auto-provisioned at setup; this endpoint mints REMOTE
// runner credentials an operator enrolls on another host with
...
export const POST = withAuthenticatedUser(async (req) => {
  ...
  // Optional scope restricts a remote token to one agent's work.
  let scope: RunnerScope = null;
  if (body.scope && typeof body.scope === "object") {
    const s = body.scope as Record<string, unknown>;
    if (typeof s.agentId === "string") scope = { agentId: s.agentId };
  }
