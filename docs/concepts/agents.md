...
This is why local and remote agents connect the same way: a runner claims the work and the CLI authenticates *as the run*. There is no long-lived per-agent API key to issue, rotate, or leak — programmatic management of Harbour itself uses an [API key](../management-guide.md) instead.
