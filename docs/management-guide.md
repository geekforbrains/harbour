...
- **Env Vars** — encrypted key-value pairs (API keys, tokens) injected into the runs of jobs they're pinned for or attached to, decrypted at runtime.
...
2. `POST /api/agents` — create the agent (projectId + name + cli). The agent has no credential of its own; nothing to save
