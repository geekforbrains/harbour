...
1. `POST /api/agents` — create the agent (`projectId` + `name` + `cli`). The agent has no Harbour credential of its own
1. `POST /api/agents/:id/jobs` — create a job with schedule and instructions
1. `POST /api/docs` — create any docs the agent needs
1. `POST /api/jobs/:id/docs` — link docs to the job (or pin them for the whole project)
1. `POST /api/env-vars` — create any job-context Secrets (API keys, tokens)
1. `POST /api/jobs/:id/env-vars` — link those Secrets to the job
...
1. No runner setup needed for `local` placement — the auto-provisioned local runner claims it. To pin it to another machine, give the workflow a `placement` label and `POST /api/runners` to mint a remote runner for that label, then run the returned `connect` command on that host
1. Optionally link docs/env vars for context (passed via stdin JSON)

### Respond to a waiting run

1. `GET /api/runs?filter=waiting` — find runs needing input
1. `GET /api/runs/:id` — read the activity log to understand what the agent needs
1. `POST /api/runs/:id/activity` — post your response (auto-transitions to `pending`)

### Check system status

1. `GET /api/agents` — see every agent and its poll status (add `?projectId=` to narrow)
1. `GET /api/runs` — see active, waiting, and recent runs
1. `GET /api/runs?filter=waiting` — see what needs human attention
