...
Install the agent CLI on this machine too — the runner spawns it locally, and it
advertises only what it finds on PATH. Claude Code and Codex use their normal
login or runner-service API-key environment on the remote host.
...
- The agent record, jobs, schedules, docs, Secrets (encrypted), and run history.
